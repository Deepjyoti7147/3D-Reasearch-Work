...
your_mesh = mesh.Mesh.from_file('monkey.stl') #the stl file you want to voxelize
...
#open a file name ploted.txt in write mode
f = open("ploted.txt", "w")
...
for a in range (len(voxel)):
    ...
    f.write(str(x_points[a])  + " " + str(y_points[a]) + " "+  str(z_points[a]) + "\n" )
